...
boolean settingsRead() {
  ...
  strlcpy(wifi_ssid, "FLIA PAZOS", sizeof(wifi_ssid));
  strlcpy(wifi_password, "8245662s", sizeof(wifi_password));
  strlcpy(mqtt_server, "192.168.18.102", sizeof(mqtt_server));
  strlcpy(mqtt_cloud_id, "3b9e2706-54be-43bb-90cd-88d70c455223", sizeof(mqtt_cloud_id));
  mqtt_cloud_enable = true;
  mqtt_time_send = true;
  mqtt_time_interval = 2000;
...
void settingsReset() {
  ...
  strlcpy(device_id, "3b9e2706-54be-43bb-90cd-88d70c455223", sizeof(device_id));
  wifi_ip_static = false;
  strlcpy(wifi_ssid, "FLIA PAZOS", sizeof(wifi_ssid));
  strlcpy(wifi_password, "8245662s", sizeof(wifi_password));
